...
`).map(s=>s.trim()).filter(Boolean);let o=[];a?o=i.map(s=>{const d=s.split("|").map(c=>c.trim());return{label:d[0]||"",url:d[1]||"",topic:"",soru:parseInt(d[2])||0}}):o=i.map(s=>{const d=s.split("|").map(c=>c.trim());return{label:d[0]||"",soru:parseInt(d[1])||0}});const r={exam_type:document.getElementById("rmExam").value,subject:t,publisher:document.getElementById("rmPublisher").value.trim(),year:new Date().getFullYear(),name:e,tests:o,active:document.getElementById("rmActive").value==="true",resource_type:a?"playlist":"book"};n?(await f.from("resources").update(r).eq("id",n),h("Güncellendi ✓")):(await f.from("resources").insert(r),h("Kaynak eklendi ✓")),K("resourceModal"),Ve()}async function io(e){await X("Bu kaynağı silmek istediğinizden emin misiniz?")&&(await f.from("resources").delete().eq("id",e),h("Silindi"),Ve())}async function ut(){const e=document.getElementById("view-dev-finance"),[{data:t},{data:n}]=await Promise.all([f.from("subscriptions").select("*,users(full_name,color)").order("created_at",{ascending:!1}),f.from("payments").select("*,users(full_name)").order("payment_date",{ascending:!1}).limit(20)]),a=(n||[]).filter(o=>o.status==="completed").reduce((o,r)=>o+Number(r.amount),0),i=(t||[]).filter(o=>o.status==="active").length;e.innerHTML=`
    ...
    </div>`,document.body.appendChild(e),e.addEventListener("click",t=>{t.target===e&&e.classList.remove("open")})),document.getElementById("pmStudent").innerHTML=l.students.map(t=>`<option value="${t.id}">${v(t.name)}</option>`).join(""),N("paymentModal")}async function so(){const e=parseFloat(document.getElementById("pmAmount").value);if(!e)return h("Tutar girin!");await f.from("payments").insert({student_id:document.getElementById("pmStudent").value,amount:e,method:document.getElementById("pmMethod").value,payment_date:document.getElementById("pmDate").value,description:document.getElementById("pmDesc").value,status:"completed"}),h("Ödeme kaydedildi ✓"),K("paymentModal"),ut()}function ro(){let e=document.getElementById("subModal");e||(e=document.createElement("div"),e.id="subModal",e.className="modal-bg",e.innerHTML=`<div class="modal"><button class="modal-close" onclick="cm('subModal')">×</button>
      <h2>Abonelik Ekle</h2>
  ...
  `;const o=document.getElementById("devChatMessages");o&&(o.scrollTop=o.scrollHeight)}async function fo(){const e=document.getElementById("devReplyInput"),t=e.value.trim();if(!t||!ie)return;e.value="",A(!0);const{data:n}=await f.from("tickets").select("body").eq("id",ie).single();let a=[];if(n&&n.body)try{a=JSON.parse(n.body)}catch{a=[{sender:"user",text:n.body,time:new Date().toISOString()}]}const i={sender:"emin",text:t,time:new Date().toISOString(),name:"Kurucu / Destek"};a.push(i);const{error:o}=await f.from("tickets").update({body:JSON.stringify(a),reply:t,status:"resolved",updated_at:new Date().toISOString()}).eq("id",ie);if(A(!1),o){h("Hata: "+o.message);return}h("Yanıt gönderildi ✓"),Pe()}async function xo(e,t){await f.from("tickets").update({status:t,updated_at:new Date().toISOString()}).eq("id",e),h("Durum güncellendi ✓"),Pe()}async function bo(e){await X("Bu talebi silmek istediğinizden emin misiniz?")&&(await f.from("tickets").delete().eq("id",e),h("Silindi"),ie=null,Pe())}function ho(){gt()}async function gt(){let e=document.getElementById("supportChatModal");e||(e=document.createElement("div"),e.id="supportChatModal",e.className="modal-bg",e.style.zIndex="99999999",e.innerHTML=`
    ...
    </div>`,i.appendChild(r)}),document.body.appendChild(i),setTimeout(()=>i.remove(),8e3)}(()=>{const e=document.createElement("style");e.textContent=".role-dev{background:rgba(192,132,252,.15);color:#c084fc;}",document.head.appendChild(e)})();function Eo(){let e=document.getElementById("onboardingModal");e||(e=document.createElement("div"),e.id="onboardingModal",e.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.85);z-index:9000;display:flex;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(8px)",document.body.appendChild(e)),dt(0,e)}const lt=[{icon:"🎉",title:"Rostrum Akademi'ye Hoş Geldiniz!",body:"Koçluk platformunuzu birkaç adımda kuruyoruz. Sadece 3 dakika.",fields:[],nextLabel:"Başlayalım →"},{icon:"🏷️",title:"Markanızı Tanıyalım",body:"Öğrencileriniz uygulamaya girdiğinde ne görsün?",fields:[{id:"ob_brand",label:"Akademi / Koçluk Adı",placeholder:"Örn: Ayşe Koçluk, EminHoca Akademi",type:"text"},{id:"ob_color",label:"Marka Rengi",type:"color",value:"#f0a500"}],nextLabel:"Devam →"},{icon:"👤",title:"Koç Profiliniz",body:"Öğrenci eşleştirme ve profil sayfanız için birkaç bilgi.",fields:[{id:"ob_phone",label:"Telefon Numarası (isteğe bağlı)",placeholder:"05XX XXX XX XX",type:"tel"},{id:"ob_examtypes",label:"Uzmanlık Alanlarınız",type:"examtypes"},{id:"ob_studentcount",label:"Kaç öğrenciyle çalışıyorsunuz?",type:"studentcount"}],nextLabel:"Devam →",skipLabel:"Şimdilik Geç"},{icon:"🔐",title:"Şifrenizi Belirleyin",body:"Güvenli bir giriş şifresi oluşturun.",fields:[{id:"ob_pass1",label:"Yeni Şifre",placeholder:"En az 8 karakter",type:"password"},{id:"ob_pass2",label:"Şifre Tekrar",placeholder:"Aynı şifreyi girin",type:"password"}],nextLabel:"Devam →",skipLabel:"Şimdilik Geç"},{icon:"👨‍🎓",title:"İlk Öğrencinizi Ekleyin",body:"Şimdi ya da sonra ekleyebilirsiniz.",fields:[{id:"ob_stuname",label:"Öğrenci Adı Soyadı",placeholder:"Muzaffer Sabri Koçar",type:"text"},{id:"ob_stuuser",label:"Kullanıcı Adı",placeholder:"muzaffer",type:"text"},{id:"ob_stupass",label:"Öğrenci Şifresi",placeholder:"ogrenci123",type:"text"}],nextLabel:"Devam →",skipLabel:"Şimdilik Geç"},{icon:"✅",title:"Platformunuz Hazır!",body:"Kurulum tamamlandı. İşte başlayabileceğiniz 3 temel özellik:",fields:[],nextLabel:"Panele Git →",isCompletion:!0}];function dt(e,t){const n=lt[e],a=lt.length,i=Array.from({length:a},(s,d)=>`<div style="width:${d===e?24:8}px;height:8px;border-radius:99px;background:${d===e?"var(--accent)":"var(--border2)"};transition:width .3s"></div>`).join("");function o(s){if(s.type==="color")return`<div class="field"><label>${s.label}</label>
    ...
    <div style="display:flex;gap:6px;justify-content:center;margin-top:20px">${i}</div>
      ...
      </div>`}let _="";if(m.length>0){const T=u.type,$=(EXAM_DEFS[T]||[]).map(M=>{var B;const D=m.filter(C=>C.type===T).map(C=>{var Y;return Number(((Y=C.nets)==null?void 0:Y[M])||0)}),j=D.length?D.reduce((C,Y)=>C+Y,0)/D.length:0,w=Number(((B=u.nets)==null?void 0:B[M])||0);return{f:M,avg:j.toFixed(1),last:w,color:It(w)}});_=`
      <div class="card cp" style="margin-bottom:16px">
...
🔗 ${n}`,o=`https://api.whatsapp.com/send?text=${encodeURIComponent(a)}`;window.open(o,"_blank"),K("reportModal"),h("WhatsApp yönlendirmesi açıldı ✓")}function Ro(){let e=document.getElementById("weeklyPDFModal");e||(e=document.createElement("div"),e.id="weeklyPDFModal",e.className="modal-bg",e.innerHTML=`<div class="modal">
  ...
  <div class="page-break" style="padding:40px 30px;max-width:1000px;margin:0 auto">
    ...
    `)}function Uo(){const e=document.getElementById("aiChatPanel"),t=document.getElementById("aiChatBubble");if(e.classList.contains("open"))e.classList.remove("open"),t.style.display="flex";else{e.classList.add("open"),t.style.display="none";const n=document.getElementById("aiMessages");n.scrollTop=n.scrollHeight,document.getElementById("aiInput").focus()}}function Wo(e){document.getElementById("aiInput").value=e,Vn()}function St(){var t;const e={};try{const n=l.students.find(r=>r.id===l.activeStuId);n&&(e.studentName=n.name,e.target=n.target||""),b.role==="parent"&&b.childName&&(e.studentName=b.childName);const a=(l.exams||[]).filter(r=>r.studentId===l.activeStuId).slice(-5);a.length&&(e.recentExams=a.map(r=>({name:r.type+" "+(r.name||""),date:r.date||"",nets:r.nets||{}})));let i=[];if(Object.entries(l.tasks||{}).forEach(([r,s])=>{r.startsWith(l.activeStuId+"_")&&(i=i.concat(s))}),i.length){const r=i.filter(s=>s.done).length;e.taskCompletionRate=Math.round(r/i.length*100),e.weeklyTaskCount=i.length}const o=Object.keys(EXAM_DEFS);a.length&&(e.examProfile=((t=a[0])==null?void 0:t.type)||o[0])}catch(n){console.warn("AI context error:",n)}return e}function $e(e,t){vt.push({role:e,content:t});const n=document.getElementById("aiMessages"),a=n.querySelector(".ai-welcome");a&&a.remove();const i=new Date().toLocaleTimeString("tr-TR",{hour:"2-digit",minute:"2-digit"}),o=document.createElement("div");o.className=`ai-msg ${e}`,o.innerHTML=`${v(t).replace(/\n/g,"<br>")}<div class="ai-msg-time">${i}</div>`,n.appendChild(o),n.scrollTop=n.scrollHeight}async function Vn(){if(wt)return;const e=document.getElementById("aiInput"),t=e.value.trim();if(t){e.value="",$e("user",t),wt=!0,document.getElementById("aiTyping").classList.add("show"),document.getElementById("aiSendBtn").disabled=!0;try{const n=St(),a=b.role||"student",i=(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1","/api/ai-chat"),o=await fetch(i,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:vt.slice(-10),context:n,userRole:a})});if(o.ok){const r=await o.json();$e("assistant",r.reply||"Yanıt alınamadı.")}else{const r=await Le(t,n,a);$e("assistant",r)}}catch(n){console.error("AI error:",n);try{const a=St(),i=await Le(t,a,b.role||"student");$e("assistant",i)}catch{const i=localStorage.getItem("gemini_api_key");$e("assistant","🔒 Bu özellik ileride aktif olacaktır. Yakında kullanıma açılacak.")}}finally{wt=!1,document.getElementById("aiTyping").classList.remove("show"),document.getElementById("aiSendBtn").disabled=!1}}}let $t=null;async function Zn(e){try{const t=await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${e}`);if(!t.ok)return null;const a=(await t.json()).models||[];let i=a.find(o=>o.name.toLowerCase().includes("flash")&&o.supportedGenerationMethods.includes("generateContent"));if(i||(i=a.find(o=>o.supportedGenerationMethods.includes("generateContent"))),i)return i.name.replace("models/","")}catch(t){console.warn("Auto-detect model failed:",t)}return null}async function Le(e,t,n){var p,m,u,y,E,I;let a=localStorage.getItem("gemini_api_key");if(!a)try{const{data:g}=await f.from("platform_settings").select("value").eq("key","ai_settings").maybeSingle();g&&g.value&&g.value.gemini_api_key&&(a=g.value.gemini_api_key)}catch(g){console.warn("DB Gemini API key load error:",g)}const i=a;if(!i)throw new Error("API anahtarı eksik.");let o="gemini-1.5-flash";if(i)if($t)o=$t;else{const g=await Zn(i);g&&($t=g,o=g,console.log("[Gemini API] Otomatik model tespiti başarılı:",o))}let r=`Sen "Rostrum Akademi Yapay Zeka Asistanı"sın. Türkiye eğitim sistemine (YKS, LGS) hakim, rolüne göre öğrencilere, velilere veya koçlara destek veren bir yapay zekasın.
