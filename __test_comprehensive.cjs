...
async function testCoach(browser) {
  ...
  try {
    ...
    // 2. ÖĞRENCİLERİM
    await page.locator('.tn-nav-item').filter({ hasText: /öğrenci/i }).first().click();
    await page.waitForTimeout(1500);
    ...
    if (modalOk) {
      ...
      await page.locator('#taskModal button[onclick*="saveTask"]').click();
    ...
    // 10. MESAJLAR
    const msgTab = page.locator('.tn-nav-item').filter({ hasText: /mesaj/i }).first();
    ...
    // 11. TO-DO
    const todoTab = page.locator('.tn-nav-item').filter({ hasText: /to-do|todo/i }).first();
...
async function testStudent(browser) {
  ...
  try {
    ...
    // Sidebar öğeleri
    const sbItems = await page.locator('.tn-nav-item').allTextContents();
    ...
    // Programım sekmesi
    const progTab = page.locator('.tn-nav-item').filter({ hasText: /program/i }).first();
    ...
    // Denemeler sekmesi
    const examTab = page.locator('.tn-nav-item').filter({ hasText: /deneme/i }).first();
    ...
    // Koçuma Yaz
    const msgTab = page.locator('.tn-nav-item').filter({ hasText: /koç|mesaj/i }).first();
    if (await msgTab.count() > 0) {
      await msgTab.click();
      await page.waitForTimeout(1200);
    ...
    // Profil sekmesi
    const profileTab = page.locator('.tn-nav-item').filter({ hasText: /profil/i }).first();
